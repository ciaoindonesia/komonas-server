...
import { KabupatenController } from "../controllers/KabupatenController";

const API_URL = Configurator.get('api');
const API_NAME = 'kabupaten';

let controller = new KabupatenController();
let router = Router();
...
router.post(API_URL + '/' + API_NAME + '/save', Authenticator, async (req, res) => {
    try {
        let body = req.body;
        body['provinceId'] = body['province']['id'];
